...
// import { VariantProps, cva } from "class-variance-authority" // Removed VariantProps, kept cva
...
// Removed unused ref from function arguments
...
>(({ className, ...props }, ref) => {
  return (
    <span
      ...
      className={cn(
        "duration-200 min-w-0 flex-1 truncate transition-[margin,opacity] ease-linear group-data-[collapsible=icon]:opacity-0",
        className
      )}
      {...props}
    />
...
const SidebarMenuTooltip = React.forwardRef<
  ...
  Omit<React.ComponentProps<typeof Tooltip>, "ref"> // Omit ref from Tooltip props
// Removed unused ref from function arguments
>(({ ...props }) => {
...
>(({ side = "right", ...props }, ref) => (
  <TooltipContent ref={ref} side={side} {...props} />
))
